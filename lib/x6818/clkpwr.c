...
void clkpwr_pll_set_freq(pll_index_t index, pll_freq_t freq) {
	clkpwr_regset_t *reg_set = (clkpwr_regset_t *) PHY_BASEADDR_CLKPWR_MODULE;
	pllSetReg_t pllsetreg_val = reg_set->pllsetreg[index];
...
void clkpwr_sys_set_bclk_freq(pll_clk_sys_index_t index, uint32_t freq) {
	clkpwr_regset_t *reg_set = (clkpwr_regset_t *) PHY_BASEADDR_CLKPWR_MODULE;
	clkdivreg_t *clkdivreg = (clkdivreg_t *) &reg_set->DVOREG[index];
	uint32_t src_freq = pll_clk_pll_get_freq(
			(pll_index_t) clkdivreg->reg_field.clksel_bclk);
...
void clkpwr_sys_set_pclk_freq(pll_clk_sys_index_t index, uint32_t freq) {
	clkpwr_regset_t *reg_set = (clkpwr_regset_t *) PHY_BASEADDR_CLKPWR_MODULE;
	clkdivreg_t *clkdivreg = (clkdivreg_t *) &reg_set->DVOREG[index];
	uint32_t src_freq = pll_clk_pll_get_freq(
			(pll_index_t) clkdivreg->reg_field.clksel_bclk);
...
void clkpwr_pll_init(void)
{
	clkpwr_pll_set_freq(PLL_INDEX_0, PLL01_FREQ_800MHZ);
	clkpwr_pll_set_freq(PLL_INDEX_1, PLL01_FREQ_800MHZ);
	clkpwr_pll_set_freq(PLL_INDEX_2, PLL23_FREQ_800MHZ);
	clkpwr_pll_set_freq(PLL_INDEX_3, PLL23_FREQ_800MHZ);
	clkpwr_sys_set_bclk_freq(PLL_CLK_SYS_CPU0, 800);
	clkpwr_sys_set_pclk_freq(PLL_CLK_SYS_CPU0, 200);
	clkpwr_sys_set_bclk_freq(PLL_CLK_SYS_CPU1, 800);
	clkpwr_sys_set_pclk_freq(PLL_CLK_SYS_CPU1, 200);
}
